...
export async function handleStartCommand(ctx: Context) {
  return ctx.reply(
    ...
    `Делитесь своей реферальной ссылкой с друзьями и получайте награды! Каждый приглашенный реферал - 1000 FLOPS!`
  );
}
...
export async function handleHelpCommand(ctx: Context) {
  return ctx.reply(
    ...
    `*Текущий розыгрыш:*\n` +
    `Первые 5 пользователей, которые пригласят ${LEADERBOARD_LIMIT}+ человек, выигрывают 20000 FLOPS!\n\n` +
